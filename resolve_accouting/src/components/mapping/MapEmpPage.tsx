...
const MapEmpPage: React.FC = () => {
  ...
  // Set selected ledger head for each payroll item when the page loads
  useEffect(() => {
    if (payrollItems.length > 0 && activeLedgerHeads.length > 0) {
      payrollItems.forEach(item => {
        const existingMapping = getExistingMapping(item.id);
        if (existingMapping) {
          setSelectedLedgerHead(existingMapping.ledgerHeadId);
        }
      });
    }
  }, [payrollItems, activeLedgerHeads, payrollMappings]);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="card p-4 mb-6">
        <p className="text-gray-600">
          Map your employees to corresponding ledger heads to ensure accurate accounting entries. 
          Each employee should be mapped to an appropriate ledger head.
        </p>
      </div>
